...
func learnWebProgramming() {
	...
	go func() {
		serverAddress := fmt.Sprintf(":%d", getWebServerPort())
		err := http.ListenAndServe(serverAddress, pair{})
		fmt.Println("After the thingy")
...
func requestServer() {
	resp, err := http.Get(fmt.Sprintf("http://localhost:%d", getWebServerPort()))
	if err != nil {
	fmt.Println(err)
	}
